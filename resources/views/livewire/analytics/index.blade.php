    ...
    <div class="w-80 bg-gray-100 border-r border-gray-200 flex flex-col sticky top-0 h-190">

        <nav class="flex-grow p-4 space-y-">
        ...
        <main class="flex-1 overflow-auto p-6 bg-white">
           ...
           <div x-show="currentPage === 'Articles'" class="w-full">
            ...
            <div class="grid grid-cols-5 gap-6 mb-8">
                ...
                <div class="border-1 border-gray-200 p-6 rounded-lg flex flex-col items-center ">
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $totalViews }}</p></p>
                </div>
                <div class="border-1 border-gray-200 p-6 rounded-lg  flex flex-col items-center ">
                    ...
                    <p class="text-3xl font-bold text-gray-900">{{ $totalReads }}</p>
                </div>
                <div class="border-1 border-gray-200 p-6 rounded-lg  flex flex-col items-center ">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 ">
                <div class="md:col-span-2 p-6 rounded-lg border-1 border-gray-200" x-data="chartHandler()">
                    ...
                    <div class="mt-6 flex flex-wrap justify-center gap-3">
                        ...
                        <button @click="updateChart('All')" 
                                :class="activeTab === 'All' ? 'bg-gray-800 text-white shadow-lg' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'"
                                class="px-4 py-1.5 rounded-full text-sm font-semibold transition-all duration-200">
                            All
                        </button>

                        <button @click="updateChart('Views')" 
                                :class="activeTab === 'Views' ? 'bg-purple-100 text-purple-700 ring-2 ring-purple-500' : 'text-gray-500 hover:bg-gray-50'"
                                class="flex items-center px-4 py-1.5 rounded-full text-sm font-medium transition-all">
                            <span class="w-2 h-2 rounded-full bg-purple-600 mr-2"></span>
                            Views
                        </button>

                        <button @click="updateChart('Reads')" 
                                :class="activeTab === 'Reads' ? 'bg-orange-100 text-orange-700 ring-2 ring-orange-500' : 'text-gray-500 hover:bg-gray-50'"
                                class="flex items-center px-4 py-1.5 rounded-full text-sm font-medium transition-all">
                            <span class="w-2 h-2 rounded-full bg-orange-500 mr-2"></span>
                            Reads
                        </button>

                        <button @click="updateChart('Likes')" 
                                :class="activeTab === 'Likes' ? 'bg-green-100 text-green-700 ring-2 ring-green-500' : 'text-gray-500 hover:bg-gray-50'"
                                class="flex items-center px-4 py-1.5 rounded-full text-sm font-medium transition-all">
                            <span class="w-2 h-2 rounded-full bg-green-500 mr-2"></span>
                            Likes
                        </button>

                        <button @click="updateChart('Dislikes')" 
                                :class="activeTab === 'Dislikes' ? 'bg-red-100 text-red-700 ring-2 ring-red-500' : 'text-gray-500 hover:bg-gray-50'"
                                class="flex items-center px-4 py-1.5 rounded-full text-sm font-medium transition-all">
                ...
                @forelse($mostReadArticles as $article)
                    @php
                        ...
                        $seconds = $article->total_duration ?? 0;
                        if ($seconds > 3600) {
                            $durationText = round($seconds / 3600, 1) . ' hrs'; 
                        } elseif ($seconds > 60) {
                            $durationText = round($seconds / 60, 0) . ' mins'; 
                        } else {
                            $durationText = $seconds . ' sec'; 
                        }

                        
                        $widthPercent = ($maxDuration > 0) ? ($seconds / $maxDuration) * 100 : 0;
                    @endphp

                    <div class="mt-5">
                        <div class="flex justify-between text-sm text-gray-800 mb-1">
                            <span class="truncate pr-2" title="{{ $article->title }}">
                                {{ Str::limit($article->title, 25) }}
                            </span>
                            <span class="font-bold whitespace-nowrap">{{ $durationText }}</span>
                        </div>
                        
                        <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div class="h-full bg-purple-600 rounded-full transition-all duration-1000 ease-out"
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex -space-x-2 overflow-hidden">
                                        <div class="inline-block h-6 w-6 rounded-full ring-2 ring-white bg-gray-400 flex items-center justify-center text-xs text-white">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $article->views->where('duration_seconds', '>=', 30)->count() }}
                                </td>
           ...
           <div class="grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
            ...
            <div class="md:col-span-8 border border-gray-200 p-6 rounded-lg bg-white">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="md:col-span-2 bg-white rounded-lg border border-gray-100 p-1 relative">
                        <div id="map" style="height: 300px; width: 100%;"></div>
                        
                    </div>

                    <div class="md:col-span-1">
                        <div class="space-y-4 h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                            @foreach($topLocations as $location)
                                <div class="flex flex-col group cursor-pointer">
                                    <div class="flex justify-between items-center mb-1">
                                        <span class="text-sm font-medium text-gray-700 flex items-center">
                                            <span class="mr-2 text-lg">{{ $location['flag'] }}</span>
                                            {{ $location['country'] }}
                                        </span>
                                        <span class="text-sm font-bold text-gray-900 group-hover:text-indigo-600 transition">
                                            {{ number_format($location['views']) }}
                                        </span>
                                    </div>
                                    
                                    <div class="w-full bg-gray-100 rounded-full h-1.5">
                                        <div class="bg-indigo-600 h-1.5 rounded-full transition-all duration-1000" 
                                            style="width: {{ $location['width'] }}"></div>
                                    </div>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="md:col-span-4 border border-gray-200 p-6 rounded-lg shadow-sm bg-white">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-semibold text-gray-800">Top categories</h3>
                    <a href="#" class="text-sm text-indigo-600 font-medium hover:text-indigo-800">View all</a>
                </div>

                <div class="flex justify-between text-xs font-semibold text-gray-400 border-b border-gray-100 pb-2 mb-2 uppercase tracking-wide">
                    <span>Category</span>
                ...
                @forelse($topCategories as $category)
                    <div class="flex justify-between py-3 {{ $loop->last ? '' : 'border-b border-gray-50' }}">
  ...
  <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    ...
    function chartHandler() {
        return {
            ...
            init() {
                this.rawData = @json($chartData);
                setTimeout(() => {
                    this.renderChart('Views');
                }, 100);
            },
            ...
            renderChart(type) {
                ...
                const delayPerPoint = animationDuration / this.rawData.labels.length;

                this.chart = new Chart(ctx, {
                    type: 'line',
                    data: {
                        labels: this.rawData.labels,
                        datasets: this.getDatasets(type)
                    },
                    options: {
                        responsive: true,
                        maintainAspectRatio: false,
            ...
            getDatasets(type) {
                ...
                const colors = {
                    'Views': { border: '#9333ea', bg: 'rgba(147, 51, 234, 0.1)' },
                    'Reads': { border: '#f97316', bg: 'rgba(249, 115, 22, 0.1)' },
                    'Likes': { border: '#22c55e', bg: 'rgba(34, 197, 94, 0.1)' },
                    'Dislikes': { border: '#ef4444', bg: 'rgba(239, 68, 68, 0.1)' }
                };

                const typesToShow = (type === 'All') ? ['Views', 'Reads', 'Likes', 'Dislikes'] : [type];

                typesToShow.forEach(metric => {
                    const metricData = data[metric.toLowerCase()] || []; 
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        if (typeof jsVectorMap === 'undefined') {
            console.error('JSVectorMap library not loaded! Check CDN links.');
            return;
        }
